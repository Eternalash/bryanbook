...
![](/assets/hashmap红黑树.png)JDK 1.8 中 HashMap 中除了链表节点：
...
在Java[ ](http://lib.csdn.net/base/java)8 中，如果一个桶中的元素个数超过 TREEIFY\_THRESHOLD\(默认是 8 \)，就使用红黑树来替换链表，从而提高速度。
...
HashMap 中， resize\(\) 方法的作用就是初始化或者扩容哈希表。当扩容时，如果当前桶中元素结构是红黑树，并且元素个数小于链表还原阈值 UNTREEIFY\_THRESHOLD （默认为 6），就会把桶中的树形结构缩小或者直接还原（切分）为链表结构，调用的就是 split\(\):
